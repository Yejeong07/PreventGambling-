...
const questions = [
    ...
    { question: "도박 중독을 방지하려면 어떻게 해야 할까요?", options: ["도박을 시작하지 않는다", "도박에 빠지지 않도록 경각심을 갖는다", "돈을 많이 벌 수 있다는 생각을 버린다", "계속 도박을 한다"], answer: 1 },
...
function createTeam() {
    ...
    if (teamName) {
        const newTeam = { name: teamName, members: [], score: 0 };
        teams.push(newTeam);
        displayTeams();
    } else {
        alert("팀 이름을 입력하세요.");
...
function displayTeams() {
    ...
    teams.forEach((team, index) => {
        ...
        teamButton.onclick = () => joinTeam(index);
...
function joinTeam(teamIndex) {
    ...
    currentTeam.members.push("사용자");
    document.getElementById("team-section").classList.add("hidden");
    document.getElementById("quiz-section").classList.remove("hidden");
    document.getElementById("team-name-display").textContent = `팀: ${currentTeam.name}`;
    loadQuestion();
}
...
// 퀴즈 종료
function endQuiz() {
    currentTeam.score += playerScore;
    document.getElementById("quiz-section").classList.add("hidden");
    document.getElementById("result-section").classList.remove("hidden");

    const resultsContainer = document.getElementById("team-results");
    resultsContainer.innerHTML = teams
        .sort((a, b) => b.score - a.score)
        .map(
            (team, index) =>
                `<p>${index + 1}위: ${team.name} (점수: ${team.score})</p>`
        )
        .join("");
}
